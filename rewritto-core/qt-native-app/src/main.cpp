...
constexpr LegacySettingsId kLegacySettingsSources[] = {
    {"Rewritto", "Rewritto-ide (Qt Native)"},
    {"Rewritto", "Rewritto Ide"},
    {"BlingBlink", "BlingBlink IDE"},
    {"BlingBlink", "BlingBlink-ide"},
};
...
void migrateLegacySettingsIfNeeded() {
  ...
  for (const LegacySettingsId& source : kLegacySettingsSources) {
    if (migrateFrom(QString::fromUtf8(source.org),
                    QString::fromUtf8(source.app))) {
      break;
    }
  }
}
