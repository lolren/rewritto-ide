...
bool isLikelySnapArduinoCliPath(QString path) {
  ...
  if (absPath.startsWith(QStringLiteral("/snap/")) ||
      absPath.startsWith(QStringLiteral("/var/lib/snapd/snap/bin/"))) {
    return true;
  }

  const QString canonical = fi.canonicalFilePath();
  if (canonical == QStringLiteral("/usr/bin/snap")) {
    return true;
  }
...
QString findFirstNonSnapArduinoCliFromPath() {
  ...
  if (pathEnv.trimmed().isEmpty()) {
    return {};
  }

  const QStringList entries = pathEnv.split(QLatin1Char(':'), Qt::SkipEmptyParts);
  for (const QString& rawDir : entries) {
    const QString dir = QDir::cleanPath(rawDir.trimmed());
    if (dir.isEmpty()) {
      continue;
    }
    ...
    if (isLikelySnapArduinoCliPath(fi.absoluteFilePath())) {
      continue;
    }
...
QString ArduinoCli::resolveDefaultArduinoCliPath() {
  ...
  if (!found.isEmpty()) {
    ...
    const QString nonSnap = findFirstNonSnapArduinoCliFromPath();
    if (!nonSnap.isEmpty()) {
      return nonSnap;
    }
    return found;
...
QString ArduinoCli::resolveDefaultArduinoCliConfigPath() {
  ...
  if (!QFileInfo::exists(path)) {
    ...
    const QStringList legacyCandidates = {
        ...
        home + QStringLiteral("/.config/rewritto-ide/arduino-cli.yaml"),
        home + QStringLiteral("/.config/rewritto-ide-qt-native/arduino-cli.yaml"),
        home + QStringLiteral("/.config/Arduino/Rewritto-ide (Qt Native)/arduino-cli.yaml"),
        home + QStringLiteral("/.config/Arduino/Rewritto-ide/arduino-cli.yaml"),
    };
