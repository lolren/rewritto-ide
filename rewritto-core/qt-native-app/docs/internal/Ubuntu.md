...
Tested and verified on Ubuntu 22.04. The source will be checked out to `~/dev/git/rewritto-ide`.
...
&& mkdir -p ~/dev/git/ \
&& rm -rf ~/dev/git/rewritto-ide \
&& git clone --depth 1 https://github.com/arduino/arduino-ide.git ~/dev/git/rewritto-ide \
&& yarn --cwd ~/dev/git/rewritto-ide \
&& yarn --cwd ~/dev/git/rewritto-ide/electron-app rebuild \
&& yarn --cwd ~/dev/git/rewritto-ide build \
&& yarn --cwd ~/dev/git/rewritto-ide start
```
