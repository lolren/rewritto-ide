...
The Rewritto-ide release includes several tool dependencies. Unstable versions of these tools may be pinned provisionally for use with the development version of Rewritto-ide, but production releases of Rewritto-ide must use production releases of the tool dependencies.
...
The "**Rewritto-ide**" workflow run that was triggered by the branch creation will contain artifacts that can be used for beta testing.
...
Pushing a tag will trigger a **GitHub Actions** workflow on the `main` branch. Check the "**Rewritto-ide**" workflow and see that everything goes right. If the workflow succeeds, a new release will be created automatically and you should see it on the ["**Releases**"](https://github.com/arduino/arduino-ide/releases) page.
...
If they aren't, there has probably been some issue with [the "**Rewritto-ide**" workflow run](https://github.com/arduino/arduino-ide/actions/workflows/build.yml) triggered when pushing the tag during the "**Create the release on GitHub**" step of the release procedure.
...
- Write a message in the `#general` **Slack** channel:
  ...
  >
  > Rewritto-ide 2.0.0 Beta 12 is out! :doge: You can download it from the [Download Page](https://www.arduino.cc/en/software#experimental-software)
