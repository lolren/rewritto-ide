...
The Rewritto-ide's primary settings are accessible via the **File > Preferences** menu item. These provide all the configuration capability required by the average user to develop sketches.

Rewritto-ide has some additional settings which may be of interest to advanced users who want to do things such as fine tune the behavior of the application or increase log output while investigating a problem.
...
Rewritto-ide is built on the [**Eclipse Theia** IDE framework](https://theia-ide.org/), which allows it to use [**VS Code**](https://code.visualstudio.com/) themes.
...
1. If Rewritto-ide is running, select **File > Quit** from the Rewritto-ide menus to exit all windows.
1. Create a folder named `plugins` under Rewritto-ide's configuration folder:
   - **Windows:**
     ```text
...
1. Start Rewritto-ide.
1. Select **File > Preferences** from the Rewritto-ide menus. <br />
...
If you encounter Wayland-related crashes, you can force Rewritto-ide to use the X11/XWayland backend instead of native Wayland rendering:

**Method 1: Environment Variable (Recommended)**

Set the `ELECTRON_OZONE_PLATFORM_HINT` environment variable before launching Rewritto-ide:
...
Launch Rewritto-ide with the `--ozone-platform=x11` flag:
...
- [Issue #2759: Segmentation fault when launching Rewritto-ide](https://github.com/arduino/arduino-ide/issues/2759)
