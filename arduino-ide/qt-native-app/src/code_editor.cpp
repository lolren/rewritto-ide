...
void CodeEditor::lineNumberAreaPaintEvent(QPaintEvent* event) {
  ...
  // Arduino IDE 2.x style line number area background
  ...
  // Arduino IDE 2.x style breakpoint color (teal accent)
    ...
    if (block.isVisible() && bottom >= event->rect().top()) {
      ...
      if (hasBp) {
        ...
        if (isLogpoint) {
          ...
          // Normal breakpoint - circle with Arduino IDE 2.x style
          painter.setPen(isEnabled ? Qt::NoPen : QPen(bpDisabled, 2));
