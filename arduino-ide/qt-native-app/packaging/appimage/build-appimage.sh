...
ensure_clangd_18() {
  if command -v clangd >/dev/null 2>&1; then
    cp "$(command -v clangd)" "${APPDIR}/usr/bin/clangd"
    return 0
  fi
  ...
  download_and_extract_pkg() {
    ...
    (cd "${debdir}" && apt-get download "${pkg}")
    local deb
  ...
  if [[ ! -x "${clangd_src}" ]]; then
    ...
    download_and_extract_pkg clangd-18
    download_and_extract_pkg libclang-cpp18
    download_and_extract_pkg libllvm18
    download_and_extract_pkg libabsl20220623t64
    download_and_extract_pkg libgrpc++1.51t64
    download_and_extract_pkg libgrpc29t64
    download_and_extract_pkg libprotobuf32t64
    download_and_extract_pkg libprotoc32t64
    download_and_extract_pkg libclang-common-18-dev
  fi
  ...
  if [[ ! -x "${clangd_src}" ]]; then
    echo "ERROR: clangd-18 could not be prepared (missing ${clangd_src})"
    return 1
  fi
...
(cd "${WORKDIR}" && APPIMAGE_EXTRACT_AND_RUN=1 \
  "${APPIMAGETOOL}" "${APPDIR}" "Rewritto_Ide-x86_64.AppImage")
