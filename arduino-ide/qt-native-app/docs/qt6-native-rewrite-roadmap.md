...
This document is for a **full native Qt6 UI rewrite** of Arduino IDE 2.x on **Linux**, producing an **AppImage**.
...
Arduino IDE 2.x today is **Theia + Monaco + VS Code extension hosting**, packaged with Electron. A “full Qt UI rewrite” means replacing **all** of the frontend (and usually a large chunk of UI-adjacent services). That is a multi-month (often multi-year) effort depending on parity expectations, especially for:
