...
Arduino IDE 2.x is a Theia (web) application packaged as a desktop app via **Electron**. A true “native Qt Widgets/QML rewrite” would effectively be a new IDE (and would lose core capabilities like the existing Theia/Monaco UX and VS Code extension hosting).
...
Acceptance:
- End-user install/run experience matches Arduino IDE expectations.
